...
1. `--from_requests` (default): re-score the Geoapify responses already stored in
   the `geo_requests` column of a resolve output, using the corrected selection
   rules in resolve.py. This recovers every audit fix that changes how a response
   is *read* rather than what was *asked*:
     - the best-confidence feature now wins (AUDIT A1) and every field is read
       off that one feature (AUDIT P2);
     - the accepted-city filter now uses the symmetric neighbour-state set, so
       features previously discarded as "not a nearby city" are now kept
       (AUDIT P1) — this is where most of the recovery comes from;
     - zipcode->county goes through uszips.csv, mapping every returned postcode
       instead of only the modal one (AUDIT C3).
...
What canNOT be recovered offline: fixes that change the *query string* — better
fuzzy city spelling (A2), the newly reachable street markers (A3), first-ad
zipcode scoping (A4) and the wider neighbour-state suffixes (P1) all generate
addresses that were never sent, so they have no stored response. The report
prints how many such candidates exist so the un-recoverable share is explicit.
...
from resolve import format_str_address, select_geoapify, summarize
...
def report(df, US_DATA, from_addresses=False):
    ...
    print("NOTE: candidates generated by the query-CHANGING fixes (A2/A3/A4/P1)")
    print("      were never sent, so they have no stored response at all and do")
    print("      not appear in this file. Re-running extract shows how many.")
